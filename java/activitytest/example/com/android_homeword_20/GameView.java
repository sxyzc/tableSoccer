    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < mPlayerList.size(); i++) {
            ...
            if (ball.check(player)) {
                ...
                if (to == -1) {
                    ...
                    ball.changeV(PlayerList.get(topos));
                    continue;
                } else if (to == -2) {
                    Log.d("gameTest", "onDraw: 敌方进球了!");
                    score++;
                    scored_who = 1;
                    scored_time = 1;
        ...
        for (int i = 0; i < PlayerList.size(); i++) {
            ...
            if (ball.check(player)) {
                ...
                if (to == -1) {
                    int topos = (int) (Math.random() * 5) + 10;
                    ball.changeV(mPlayerList.get(topos));
                    continue;
                } else if (to == -2) {
                    Log.d("gameTest", "onDraw: 我方进球了!");
                    mScore++;
                    scored_who = 0;
                    scored_time = 1;
     ...
     public boolean onTouchEvent(MotionEvent event) {
         ...
         switch (event.getAction()) {
             case MotionEvent.ACTION_DOWN: {
                 ...
                 //看看这里的右侧数值有没有问题// Log.d("aaa", "onTouch: "+"  "+eventX+"  "+lastX);
                 break;
             }
